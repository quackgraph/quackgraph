...
## File: test/integration/lifecycle.test.ts
````typescript
// Consolidating lifecycle tests into persistence.test.ts to avoid redundancy.
...
> QuackGraph adds a zero-copy, in-memory Graph Index (CSR) on top of DuckDB, giving you **O(1) traversals** and **Fluent TypeScript DX** without the JVM overhead.
...
## 💡 Why QuackGraph? (The Pitch)

**The Problem:**
1.  **Relational Databases (SQL)** are great at storage and math, but terrible at connections. Writing `WITH RECURSIVE` CTEs is painful and slow.
2.  **Graph Databases (Neo4j)** are great at connections, but heavy to deploy and they isolate your data in a proprietary silo.

**The Solution:**
QuackGraph is a **library**, not a server. It wraps your DuckDB file and accelerates it.
*   **Storage:** We use **DuckDB** (Parquet/JSON) as the atomic Source of Truth.
*   **Compute:** We maintain a transient **Rust/WASM Index** in RAM for traversals.
*   **Speed:** We bridge them with **Apache Arrow** for Zero-Copy data transfer.

**The Result:** You get the sub-millisecond traversal speed of a native Graph DB with the simplicity and analytical power of DuckDB.

---

## 📐 The Architecture: A "Split-Brain" Engine

To ensure ACID compliance without building a complex distributed system, QuackGraph follows a strict "Transient Cache" model.
...
| Feature | QuackGraph 🦆 | Neo4j | Raw DuckDB |
| :--- | :--- | :--- | :--- |
...
export class QueryBuilder {
  ...
  async select<T = any>(projection?: string | ((node: any) => T)): Promise<T[]> {
    ...
    if (this.vectorSearch) {
      ...
      orderBy = `ORDER BY array_distance(embedding::FLOAT[], ${vectorStr}::FLOAT[])`;
